...
DEFAULT_GHIDRA_SERVER = "http://127.0.0.1:8888/"
...
# Extra helper functions not in Laurie's original
@mcp.tool()
def get_function_address_by_name(name: str) -> str:
    ...
    Returns:
        Address of the function as a string
    """
    return "\n".join(safe_get("get_function_address_by_name", {"name": name}))

@mcp.tool()
def check_connection() -> str:
    """
    Check if the Ghidra plugin is running and responsive.
...
# Additional endpoints available in the Java plugin
@mcp.tool()
def get_metadata() -> str:
    """
    Get metadata about the current program including architecture, memory info, and function count.
    
    Returns:
        Detailed metadata about the current Ghidra program
    """
    return "\n".join(safe_get("get_metadata"))

@mcp.tool()
def list_function_variables(function_name: str) -> str:
    """
    List all variables (local variables and parameters) for a specific function.
    
    Args:
        function_name: Name of the function to get variables for
        
    Returns:
        List of variables with their types and addresses/ordinals
    """
